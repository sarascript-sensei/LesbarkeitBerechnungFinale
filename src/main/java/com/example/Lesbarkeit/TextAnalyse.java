...
public class TextAnalyse {
    ...
    public double getFleschReadingEase(){
        double score = 180 - (double) getNumberOfWords()/getNumberOfSentences() - 58.5 * SilbenAnzahl.countSyllables(content)/getNumberOfWords();
    ...
    public double getFlesch_Kincaid_Grade_Level(){
        double score = (0.39 * (double) getNumberOfWords()/getNumberOfSentences()) + (11.8 * SilbenAnzahl.countSyllables(content)/getNumberOfWords()) - 15.59;
        return round(score, 3);
    }
    ...
    public double calculateWienerSachtextformel() {
        float ms = (float) (SilbenAnzahl.percentageOfWordsWithThreeOrMoreSyllables(content));
        double sl = averageNumberOfWordsPerSentence();
        float iw = percentageOfWordsWithSixOrMoreMoreCharacters() * 100;
        float es = (float) SilbenAnzahl.percentageOfWordsWithOneSyllable(content);
